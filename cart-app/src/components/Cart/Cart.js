  ...
  const total = cartItems.reduce((a,c)=> a + c.price * c.qty,0);
    return (
      <div className="col-4 bg-light">
        ...
        {countCartItems ? 
        (<h1><span className="badge bg-secondary">{countCartItems}</span></h1>) : ("")
        }
        ...
        <div>
          ...
          {cartItems.map((item, index) => (
            <div key={item.id} className="alert alert-secondary">
              ...
              <div>
                ...
                className="btn btn-success btn mx-1">+</button>
                <button onClick={() => onRemove(item)}
                className="btn btn-danger btn mx-1">-</button>
              </div>
              <div>
          ...
          {cartItems.length !== 0 && (
            <>
              <hr />
              <div>total is ₹ {total.toFixed(2)}</div>
            </>
          )}
